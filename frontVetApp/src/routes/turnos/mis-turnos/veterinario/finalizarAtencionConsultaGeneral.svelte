<script lang="ts">
    ...
    const TurnoFinalizado: ModalSettings = {
        type: 'alert',
        title: 'Turno con motivo Consulta General finalizado',
    ...
    async function onFormSubmit() {
        await fetch("http://localhost:3000/turnos/finalizar-turno-consultaGeneral",{
                method:"POST",
                headers:{
                    "Content-Type":"application/json",
                },
...
{#if $modalStore[0]}
	<div class="modal-example-form {cBase}">
		<header class={cHeader}>Finalizar atención Consulta General</header>

<script lang="ts">
    ...
    const TurnoFinalizado: ModalSettings = {
        type: 'alert',
        title: 'Turno con motivo Antiparasitación finalizado',
    ...
    async function onFormSubmit() {
        await fetch("http://localhost:3000/turnos/finalizar-turno-antiparasitacion",{
                method:"POST",
                headers:{
                    "Content-Type":"application/json",
                },
...
{#if $modalStore[0]}
	<div class="modal-example-form {cBase}">
		<header class={cHeader}>Finalizar atención Antiparasitación</header>
        ...
        <form class="modal-form {cForm}" on:submit|preventDefault={onFormSubmit}>  
            ...
            <label class="label">
                ...
                <input class="input" bind:value={nombreAntip} title="Input (text)" type="text" on:change={actualizarAnti} required/>
            </label>
